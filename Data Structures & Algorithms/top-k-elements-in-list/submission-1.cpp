...
public:
   ...
   vector<int> findResult(vector<int>& nums, int k) {
    ...
    vector<int> findResult3(vector<int>& nums, int k) {
        vector<int> result;
        unordered_map<int, int> counts;
        ...
        for(int key=maxFreq; key>=0 && result.size()<k; key--) {
            if(!buckets.contains(key)) {
                continue;
            }
           // cout<<key<<buckets[key].size();
            for(int num:buckets[key]) {
                if(result.size()==k) {
                    break;
                }
